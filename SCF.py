...
class SCF:
    ...
    def build_E(self) -> None:
        ...
        self.E = np.zeros((len(self.basis_set.cgtos), len(self.basis_set.cgtos), len(self.basis_set.cgtos), len(self.basis_set.cgtos)))

        for i, A in enumerate(self.basis_set.cgtos):
            for j, B in enumerate(self.basis_set.cgtos):
                for k, C in enumerate(self.basis_set.cgtos):
                    for l, D in enumerate(self.basis_set.cgtos):

                        self.E[i][j][k][l] = twoel_cgto(A, B, C, D)
